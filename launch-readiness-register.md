...
- **Status:** Done [2026-05-17, Wave B Session 2 — Canonical helper landed at `adgm_corpus.storage._section_ref`. `decompose_section_ref(section_ref)` returns a format-discriminated `DecomposedSectionRef` dataclass (frozen). `extract_rulebook_code(section_ref)` is the lightweight convenience helper. **Brief originally proposed 5 formats** based on what `structure_parser.py` currently generates; Step 2 discovery surfaced 6 additional shapes from legacy parser versions and ad-hoc corpus additions — expanded to **11 formats** to match actual corpus state (claim-vs-landed per Memory #28). Format shapes: `fsra_module` (FSRA/DFSA rulebooks, including numeric paren-provision `(1)`), `legislation_en` (DIFC/ADGM ER/WB/CR/CLR, with `s.N` aliased to canonical `Sec.N`, and capital `PART N descriptive` normalised to `Part`), `legislation_es` (El Salvador + Panama, with `§N(a)` paragraph-with-provision support and `[A-Z0-9]+` rulebook prefix for `PAN-LEY52`-style codes), `bvi_regulation`, `vara_rulebook` (including Roman-only top-level `VARA-ADV I` without `Part` keyword), `form` (em-dash separator `ADGM-FSRA-F — Title`), and `unknown`. 39 unit tests + 1 global classification test (`test_global_classification_coverage` ≥95% target — PASSED on actual corpus). Two pattern-ordering bugs found and fixed during Step 5: (a) ES Art/§ patterns initially language-agnostic and matched EN refs first → restricted via `(?:SV|PAN)(?:-[A-Z0-9]+)+` prefix guard; (b) EN `_LEG_EN_RE` segment_id didn't accept dot-separated numerics for `§3.2` → extended to `\d+(?:\.\d+)*[A-Z]?`. **D2=(c) FULL refactor:** 1 adgm-corpus consumer site refactored (`vara_parser.py:218` now routes through `decompose_section_ref()` with defensive split fallback). **qanun-api carry:** A5.B's "FULL refactor" scope was met in adgm-corpus but not in qanun-api — `services/change_detection_service.py:54, 200` use `section_ref.split()[0]` to extract rulebook_code, functionally correct for all 11 current formats but unprincipled. Refactoring would require either (a) cross-repo Python dep (first such coupling; contradicts Option C JSON-on-disk boundary), (b) duplicating the helper into qanun-api (creates convention-drift that A5.B exists to prevent), or (c) extracting `_section_ref.py` into a shared third package. Deferred as a code-quality carry pending a substantive cross-repo sharing decision. **Unknown-sample candidates** for future Wave B+ pattern refinement: specific shapes like DFSA `CMC 1` numeric-chapter style and BVI multi-word `BVI-ACT NO. Part I` style may still classify as `unknown` — global coverage is ≥95% but not 100%.]
- **Size:** Half-day
- **Dependencies:** None
- **Source:** A5 audit memo, 12 May 2026
...
*(M34–M36 added by the 11 June 2026 MAX-4 close-out — entry bodies on `sprint/max4-register-close`, PR #7; M36 flips **Done** per the v1.9 addendum's MAX5-CORPUS/EMBED bullet.)*
